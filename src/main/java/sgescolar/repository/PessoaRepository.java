...
public interface PessoaRepository extends JpaRepository<Pessoa, Long> {
	...
	@Query( "select p from Pessoa p where lower_unaccent(p.nome)=lower_unaccent(?1)" )
	public Optional<Pessoa> buscaPorNome( String nome );
	
	@Query( "select p from Pessoa p where lower_unaccent(p.nome) like lower_unaccent(?1)" )
